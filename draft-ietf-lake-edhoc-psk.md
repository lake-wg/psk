...
When using a resumption PSK derived from a previous EDHOC exchange:
  ...
  1. The resumption PSK MUST only be used with the same EDHOC hash function from which it was derived.
  2. Implementations MUST maintain a mapping between each resumption PSK and its originating EDHOC hash function to enforce this requirement.
  3. If a resumption PSK is combined with a different EDHOC hash function, the Responder MUST reject the ongoing EDHOC session.
